...
export function LogViewer() {
  ...
  const getLevelColor = (level: string) => {
    switch (level) {
      case "INFO":
        return "text-green-accent font-semibold";
      case "WARN":
          ...
          {(["ALL", "INFO", "WARN", "ERROR", "DEBUG"] as const).map((lvl) => (
            <button
              ...
              className={`px-2 py-0.5 rounded-xs text-[10px] uppercase font-bold transition ${
                filter === lvl
                  ? "bg-white/10 text-ink font-extrabold"
        ...
        {filteredLogs.length > 0 ? (
          filteredLogs.map((log, idx) => (
            <div key={idx} className="flex gap-2 items-start py-[1px]">
              ...
              <span className={`shrink-0 uppercase tracking-wider text-[10px] ${getLevelColor(log.level)}`}>
                [{log.level}]
              </span>
