...
export function PresenceCard({
  ...
  const getAvatarColor = (name: string) =>
    avatarColors[
      name.length > 0 ? name.charCodeAt(0) % avatarColors.length : 0
    ];
  ...
  const renderStatusIndicator = (
    ...
    switch (status) {
      case "online":
        return (
          <div
            className="w-full h-full rounded-full bg-green-accent"
            title="Online"
          />
        );
      case "idle":
        return (
          <svg
            className="w-full h-full text-yellow-accent"
            viewBox="0 0 24 24"
            fill="currentColor"
          >
      ...
      case "dnd":
        return (
          <div
            className="w-full h-full rounded-full bg-danger flex items-center justify-center"
      ...
      case "offline":
        return (
          <div
            className="w-full h-full rounded-full bg-muted-ink flex items-center justify-center"
            title="Invisible"
          >
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-center">
        <div
          className="relative w-[70px] h-[70px] rounded-full overflow-visible"
          style={{
        ...
        >
          ...
          <div
            className="absolute bottom-0 right-0 w-[20px] h-[20px] rounded-full flex items-center justify-center overflow-hidden"
            style={{
      ...
      {/* Profile Body (Card) */}
      <div
        className="mt-3 p-3 flex flex-col gap-3 z-0 relative neo-border-2"
      ...
      >
        ...
        <div className="flex flex-col items-center text-center">
          ...
          {profile.customStatus && (
            <div
              className="text-[12px] text-zinc-200 mt-2 px-2 py-1.5 italic text-left neo-border-2"
              style={{
        ...
        <div className="flex flex-col">
          ...
          <div className="flex items-start gap-3 mt-1 text-left">
            <div
              className="relative w-[60px] h-[60px] flex-shrink-0 flex items-center justify-center overflow-visible neo-border-2 neo-shadow-sm"
              style={{
                backgroundColor: "rgba(255,255,255,0.05)",
                borderRadius: "8px",
              }}
            >
              {presence ? (
                <AppIcon
                  name={presence.large_image || appName}
